...
class ISRF_Module(nn.Module):
    """
    Implicit Sub-Pixel Regression Field (ISRF) for YOLO26.
    Predicts a small, bounded residual correction (dl, dt, dr, db) on top 
    of the coarse ltrb box regression.
    """
    def __init__(self, in_channels: int, hidden_dim: int | None = None):
        super().__init__()
        hidden_dim = hidden_dim or max(16, in_channels // 2)

        # Replaces unfold+Linear with native Conv2d for NPU/INT8 export compatibility
        ...
        # Zero-init so ISRF is a no-op at step 0
        nn.init.zeros_(self.refine[-1].weight)
        nn.init.zeros_(self.refine[-1].bias)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        raw = self.refine(x)
        # tanh provides a max gradient of 0.5 at init (vs 0.25 for sigmoid)
        # Bounded to [-0.5, 0.5] grid-cell units
...
class Detect(nn.Module):
    ...
    def __init__(self, nc: int = 80, reg_max=16, end2end=False, ch: tuple = ()):
        ...
        self.isrf = nn.ModuleList(ISRF_Module(c) for c in ch)

        if end2end:
            self.one2one_cv2 = copy.deepcopy(self.cv2)
            self.one2one_cv3 = copy.deepcopy(self.cv3)
            self.one2one_isrf = copy.deepcopy(self.isrf)  # Deepcopy ISRF for end2end
    ...
    def forward_head(
        ...
        # Smart routing for end2end
        is_one2one = hasattr(self, "one2one_cv2") and box_head is self.one2one_cv2
        isrf_branch = getattr(self, "one2one_isrf", None) if is_one2one else getattr(self, "isrf", None)
        
        boxes_list = []
        isrf_list = []
        for i in range(self.nl):
            raw_box = box_head[i](x[i])
            if isrf_branch is not None:
                isrf_out = isrf_branch[i](x[i])
                box_pred = raw_box + isrf_out
                isrf_list.append(isrf_out.view(bs, 4 * self.reg_max, -1))
            else:
                box_pred = raw_box
                
            boxes_list.append(box_pred.view(bs, 4 * self.reg_max, -1))
        ...
        if isrf_list:
            preds["isrf_boxes"] = torch.cat(isrf_list, dim=-1) # Expose to loss function
        return preds
